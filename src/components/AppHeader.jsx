...
export default function AppHeader({ title, subtitle = 'Premier Global School', showSignOut = false, rightExtra = null }) {
  ...
  return (
    <>
      <header style={{
        background: 'linear-gradient(135deg, #0f0f0f 0%, #1e1e22 50%, #161617 100%)',
        ...
        <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 20px', height: 56, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ color: 'rgba(255,255,255,0.35)', fontSize: 10, fontWeight: 500, letterSpacing: 0.8, textTransform: 'uppercase', marginTop: 1 }}>{subtitle}</div>
            </div>
          </div>
      ...
      {showConfirm && (
        <div onClick={() => setShowConfirm(false)}
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: '#1C1C1E', marginBottom: 6 }}>Sign out?</div>
            <div style={{ fontSize: 13, color: '#8E8E93', marginBottom: 24 }}>
              You'll need to select your name and enter your PIN again to sign back in.
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={handleSignOut}
                style={{ flex: 1, background: '#1C1C1E', border: 'none', padding: '13px', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer', color: '#fff' }}>
                Sign out
              </button>
            </div>
          </div>
