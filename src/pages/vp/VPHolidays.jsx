...
export default function VPHolidays() {
  ...
  async function addHoliday() {
    ...
    await supabase.from('holidays').upsert({
      ...
      school_year: currentSchoolYear, created_by: 'vp'
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8F9FB', fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <VPHeader title="Holidays" rightExtra={<span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 11 }}>{schoolYearLabel}</span>} />
        ...
        <div className="hol-grid" style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 14 }}>
          ...
          <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 12, padding: 18 }}>
            ...
            <button onClick={addHoliday} disabled={addingHoliday || !holidayDate || !holidayName.trim()}
              style={{ width: '100%', background: '#1e003e', color: '#fff', border: 'none', padding: '10px', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: 'pointer', opacity: addingHoliday || !holidayDate || !holidayName.trim() ? 0.6 : 1 }}>
